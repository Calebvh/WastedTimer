...
(function () {
    ...
    function updateColors(siteTimeSeconds, weeklyTotalSeconds, dailyLimitSeconds, weeklyLimitSeconds) {
    ...
    `;
        ...
        if (data.siteTimeSeconds !== undefined && data.weeklyTotalSeconds !== undefined &&
            ...
            updateColors(data.siteTimeSeconds, data.weeklyTotalSeconds, data.dailyLimitSeconds, data.weeklyLimitSeconds);
        }
    ...
    async function handleUnminimize() {
        isMinimized = false;
    ...
    function startSnoozeCheck() {
        if (snoozeCheckInterval !== null)
            return;
        snoozeCheckInterval = window.setInterval(async () => {
            const response = await browser.runtime.sendMessage({ type: 'checkSnooze' });
    ...
    async function updateOverlay() {
        ...
        if (response) {
            ...
            const weeklyEl = document.getElementById('wasted-timer-weekly');
            if (siteEl)
                siteEl.textContent = response.siteTime;
            if (weeklyEl)
                weeklyEl.textContent = response.weeklyTotal;
            // Update colors based on limits
            updateColors(response.siteTimeSeconds, response.weeklyTotalSeconds, response.dailyLimitSeconds, response.weeklyLimitSeconds);
        }
    }
